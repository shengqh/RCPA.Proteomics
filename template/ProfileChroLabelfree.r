...
for (index in c(1:nrow(filelist))){
  ...
  if(outputImage){
    ...
    png(imgFile, res=300, width = 1500, height=2000)
    ...
    print(p2, vp = vplayout(2:4, 1))  # key is to define vplayout
    
    xpos<-0.06
    grid.text("(A)",x=xpos,y=0.99)
    grid.text("(B)",x=xpos,y=0.74)
    grid.text("(C)",x=xpos,y=0.49)
    grid.text("(D)",x=xpos,y=0.24)
