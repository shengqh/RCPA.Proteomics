...
{
  public partial class SummaryBuilderUI : AbstractProcessorFileUI
  {
    ...
    private RcpaCheckBox autoFindTitleParser;
    ...
    protected bool TitleVisible
    {
      get
      {
        return pnlTitleFormat.Visible;
      }
      set
      {
        pnlTitleFormat.Visible = value;
        btnFindFormat.Visible = value;
      }
    }

    public SummaryBuilderUI()
    {
      InitializeComponent();
      ...
      autoFindTitleParser = new RcpaCheckBox(cbAutoFindFormat, "AutoFindTitleFormat", true);
      AddComponent(this.autoFindTitleParser);
    ...
    protected virtual string GetTitleSample(string name)
    {
      throw new NotImplementedException("GetTitleSample");
    }

    private void btnFindFormat_Click(object sender, EventArgs e)
    {
      FindTitleFormat(true);
    }

    private void FindTitleFormat(bool byUser)
    {
      if (TitleVisible)
      {
        if (lvDatFiles.SelectedItems.Count > 0)
        {
          var name = lvDatFiles.SelectedItems[0].SubItems[0].Text;

          DateTime dt;
          if (new FileInfo(name).Exists)
          {
            dt = File.GetLastWriteTime(name);
          }
          else if (new DirectoryInfo(name).Exists)
          {
            dt = Directory.GetLastWriteTime(name);
          }
          else
          {
            return;
          ...
          var key = name + "_" + dt.ToShortTimeString();

          if (savedMap.ContainsKey(key))
          {
            titleParsers.SelectedItem = savedMap[key];
          }
          else
          {
            var sample = GetTitleSample(name);
            if (!string.IsNullOrEmpty(sample))
            {
              var guess = TitleParserUtils.GuessTitleParser(sample, titleParsers.Items);
              if (guess != null)
              {
              ...
              else
              {
                if (byUser)
                {
                  MessageBox.Show(this, MyConvert.Format("Cannot determinate corresponding title parser for\n{0}\nyou may need to choose by youself or contact author to add a new title parser.", sample), "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else if (!IgnoreError)
                {
                  if (MessageBox.Show(this, MyConvert.Format("Cannot determinate corresponding title parser for\n{0}\nyou may need to choose by youself or contact author to add a new title parser.\nIgnore this error at next time?", sample), "Error", MessageBoxButtons.YesNo, MessageBoxIcon.Error, MessageBoxDefaultButton.Button2) == System.Windows.Forms.DialogResult.Yes)
                  {
                    IgnoreError = true;
                  }
                }
    ...
    {
      if (lvDatFiles.SelectedItems.Count == 1 && autoFindTitleParser.Checked)
      {
        FindTitleFormat(false);
      }
    }
  }
}