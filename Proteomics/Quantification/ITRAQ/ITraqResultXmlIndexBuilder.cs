...
{
  public class ITraqResultXmlIndexBuilder : AbstractThreadFileProcessor
  {
    private bool force;
    public ITraqResultXmlIndexBuilder(bool force = false)
    {
      this.force = force;
    }
        ...
        using (FileStream sr = new FileStream(fileName, FileMode.Open))
        {
          ...
          while (true)
          {
            ...
            if (line.Trim().Equals("<ITraqScan>"))
            {
              ...
              while (null != (line = sr.ReadLine()))
              {
                if (line.Trim().StartsWith("</ITraqScan>"))
                {
