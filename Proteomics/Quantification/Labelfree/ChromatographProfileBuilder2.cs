...
{
  public class ChromatographProfileBuilder2 : AbstractThreadProcessor
  {
    ...
    public string GetTargetDirectory(string experimental)
    {
      var chrosDir = Path.Combine(Path.GetDirectoryName(options.OutputFile), "chros");
      if (!Directory.Exists(chrosDir))
      {
        Directory.CreateDirectory(chrosDir);
      }

      var result = Path.Combine(chrosDir, experimental);
    ...
    public string GetTargetSubDirectory(string experimental)
    {
      var targetExpDir = GetTargetDirectory(experimental);


      var result = Path.Combine(targetExpDir, "sub");
      if (!Directory.Exists(result))
      {
        Directory.CreateDirectory(result);
      }
