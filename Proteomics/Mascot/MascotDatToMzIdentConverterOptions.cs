...
{
  public class MascotDatToMzIdentConverterOptions : AbstractOptions
  {
    public const int DEFAULT_MinPeptideLength = 7;

    public MascotDatToMzIdentConverterOptions()
    {
    ...
    public string GetOutputFile(string inputFile)
    {
      ...
      {
        return Path.ChangeExtension(inputFile, ".mzid");
      }
      else
      {
        return Path.Combine(OutputDirectory, Path.GetFileNameWithoutExtension(inputFile) + ".mzid");
      }
    }
  }
}
