...
namespace RCPA.Proteomics.Sequest
{
  ...
  public class MsfDatabaseToNoredundantProcessor : AbstractMsfToNoredundantProcessor
  {
    ...
    {
      var modstr = MascotDatSpectrumParser.MODIFICATION_CHAR;
    ...
    public Dictionary<int, IIdentifiedPeptide> ParsePeptideMap(string fileName)
    {
      ...
      //读取肽段列表
      string sqlPeptide = string.Format("select pep.SpectrumID, pep.PeptideID, pep.TotalIonsCount, pep.MatchedIonsCount, pep.ConfidenceLevel, pep.Sequence, ps.ScoreValue from Peptides as pep, PeptideScores as ps where pep.PeptideID=ps.PeptideID and ps.ScoreID={0}", scoreid);
      ...
      while (peptideReader.Read())
      {
        ...
        var score = peptideReader.GetDouble(6);
