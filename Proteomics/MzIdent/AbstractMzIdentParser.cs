      ...
      foreach (var sir in idList.FindElements("SpectrumIdentificationResult"))
      {
        ...
        foreach (var sit in sir.FindElements("SpectrumIdentificationItem"))
        {
          ...
          if (bFirst) //only parse score once
          {
            ...
            spectrum.TheoreticalMass = double.Parse(sit.Attribute("calculatedMassToCharge").Value);
            spectrum.ExperimentalMass = double.Parse(sit.Attribute("experimentalMassToCharge").Value);
